Attribute VB_Name = "Mod_DX8_Projectiles_vbGore"
...
Dim ProjectileIndex As Integer
    ...
    'Fill in the values
    ProjectileList(ProjectileIndex).tX = (CharList(TargetIndex).Pos.X + IIf(Fallo = True, General_Get_Random_Number(-4, 4), 0)) * 32
    ProjectileList(ProjectileIndex).tY = (CharList(TargetIndex).Pos.Y + IIf(Fallo = True, General_Get_Random_Number(-4, 4), 0)) * 32
