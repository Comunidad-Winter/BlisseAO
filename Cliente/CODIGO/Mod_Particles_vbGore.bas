Attribute VB_Name = "Mod_DX8_Particles_vbGore"
Option Explicit
...
Private Type Effect
    ...
    PartVertex() As D3DTLVERTEX    'Used to point render particles
    ...
    For LoopC = 0 To Effect(EffectIndex).ParticleCount
        ...
        If Effect(EffectIndex).Particles(LoopC).Used Then
            ...
            Else
                ...
                Effect(EffectIndex).PartVertex(LoopC).sX = Effect(EffectIndex).Particles(LoopC).sngX
                Effect(EffectIndex).PartVertex(LoopC).sY = Effect(EffectIndex).Particles(LoopC).sngY
...
If Settings.ParticleEngine = False And bRain = False Then Exit Sub
'*****************************************************************
...
Dim LoopC As Long
    ...
    For LoopC = 1 To NumEffects
        ...
        If Effect(LoopC).Used Then
            ...
            If Settings.ParticleEngine = True Then
        ...
        With Effect(EffectIndex).Particles(LoopC)
            ...
            If .Used Then
                ...
                Else
                    ...
                    Effect(EffectIndex).PartVertex(LoopC).sX = .sngX
                    Effect(EffectIndex).PartVertex(LoopC).sY = .sngY
...
Dim i As Byte
    ...
    For i = 1 To UBound(ParticleTexture())
        ...
        Set ParticleTexture(i) = DirectD3D8.CreateTextureFromFileEx(DirectDevice, Resources.Graphics & "p" & i & ".png", D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_FILTER_POINT, D3DX_FILTER_POINT, &HFF000000, ByVal 0, ByVal 0)
    Next i
...
Sub Engine_Weather_Update()
    ...
    If OnRampageImgGrh <> 0 Then
        TileEngine_Render_GrhIndex OnRampageImgGrh, 0, 0, 0, ColorData.Blanco(), 1
    End If
...
'*****************************************************************
    ...
    If CurMapAmbient.Fog <> -1 Then
        If FogStament <> CurMapAmbient.Fog Then
            If FogStament > CurMapAmbient.Fog Then
                FogStament = FogStament - 1
            Else
                FogStament = FogStament + 1
            End If
        End If
    Else
        If FogStament <> 0 Then
            FogStament = FogStament - 1
        End If
    End If


    If FogStament = 0 Then Exit Sub
...
ElapsedTime = Engine_ElapsedTime
    ...
    CC(1) = D3DColorARGB(FogStament, 255, 255, 255)
    CC(2) = D3DColorARGB(FogStament, 255, 255, 255)
    CC(3) = D3DColorARGB(FogStament, 255, 255, 255)
    CC(0) = D3DColorARGB(FogStament, 255, 255, 255)
    For i = 1 To WeatherFogCount
        TileEngine_Render_GrhIndex 27300, (X * 512) + WeatherFogX2, (Y * 512) + WeatherFogY2, 0, CC()
        X = X + 1
    ...
    X = 0
    Y = 0
    CC(1) = D3DColorARGB(FogStament / 2, 255, 255, 255)
    CC(2) = D3DColorARGB(FogStament / 2, 255, 255, 255)
    CC(3) = D3DColorARGB(FogStament / 2, 255, 255, 255)
    CC(0) = D3DColorARGB(FogStament / 2, 255, 255, 255)
    For i = 1 To WeatherFogCount
        TileEngine_Render_GrhIndex 27301, (X * 512) + WeatherFogX1, (Y * 512) + WeatherFogY1, 0, CC()
        X = X + 1
