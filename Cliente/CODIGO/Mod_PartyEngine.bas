Attribute VB_Name = "Mod_DX8_PartyEngine"
'***************************************************
        ...
        Count = 0
            For i = 1 To 5
                If PartyMembers(i).name <> "" Then
                    Count = Count + 1
                    Engine_Render_Fill_Box 410, 20 + (Count - 1) * 50 + 5, 120, 40, D3DColorARGB(100, 0, 0, 0)
                    'DDrawTransGrhIndextoSurface HeadData(PartyMembers(i).Head).Head(3).GrhIndex, 410, 20 + (Count - 1) * 50 + 35, 1, colordata.blanco(), 0, True
