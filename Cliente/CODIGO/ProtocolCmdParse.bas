Attribute VB_Name = "Mod_Protocol_Cmd_Parse"
Option Explicit
...
'***************************************************
    ...
    If Left$(Comando, 1) = "/" Then
        ...
        #If SeguridadBlisse = 1 Then
            If CanUse_Interval(dInter.Command) Then
        #End If
        ...
        Select Case Comando
            ...
            Case "/HORA"
                Call Mod_Protocol.WriteServerTime
            ...
            Case "/AUTORIZAR"
                If notNullArguments Then
                    Call WriteAutorizar(ArgumentosRaw)
                Else
                    Call ShowConsoleMsg("Faltan parámetros. Utilice /AUTORIZAR NICKNAME.")
                End If
            ...
            Case "/EDITAME" 'Standelf
                Call WriteEditame
                
            Case "/PROMEDIO" 'Standelf
                Call WritePromedio
                
            Case "/GUARDAR" 'Standelf
                Call WriteSaveCharacter
            ...
            Case "/INVASION"
                If General_Get_GM(UserCharIndex) Then
                    Call frmInvasion.Show(vbModeless, frmMain)
                End If
           ...
           Case "/SUBASTA"
            ...
            Case "/CR"
                If General_Get_GM(UserCharIndex) Then
                    If notNullArguments Then
                        If ValidNumber(ArgumentosAll(0), eNumber_Types.ent_Byte) Then
                            Call WriteCuentaRegresiva(ArgumentosAll(0))
                        Else
                            'No es numerico
                            Call ShowConsoleMsg("Valor incorrecto. Utilice /CR tiempo, Recuerde que la cuenta va desde 0 a 255.")
                        End If
                    Else
                        'Avisar que falta el parametro
                        Call ShowConsoleMsg("Faltan parámetros. Utilice /CR tiempo.")
                    End If
                End If
                
            Case "/AMBIENTE"
                Call frmAmbientEditor.Show
                
            Case "/TESTER"
                Call frmTesterForm.Show
                
            Case "/Prestigio"
                Call WritePrestigio(UserName, 0, 0)
                
            Case "/DARPrestigio"
                If General_Get_GM(UserCharIndex) Then frmPrestigioGM.Show
                
            Case Else
                Call ShowConsoleMsg("Comando no reconocido.")
                Call Log_Protocol("Unknown Command " & Comando)
        End Select
    ...
    ElseIf Left$(Comando, 1) = "." Then 'Global
        If UserEstado = 1 Then 'Muerto
            With FontTypes(FontTypeNames.FONTTYPE_INFO)
                Call ShowConsoleMsg("¡¡Estás muerto!!", .Red, .Green, .Blue, .bold, .italic)
            End With
        ...
        Call WriteChatGlobal(mid$(RawCommand, 2))
        
    ElseIf Left$(Comando, 1) = "," Then 'Facción
        If UserEstado = 1 Then 'Muerto
            With FontTypes(FontTypeNames.FONTTYPE_INFO)
                Call ShowConsoleMsg("¡¡Estás muerto!!", .Red, .Green, .Blue, .bold, .italic)
            End With
        ...
        Call WriteChatFaccion(mid$(RawCommand, 2))
...
'
'***************************************************
    Call General_Add_to_RichTextBox(frmMain.RecTxt, Message, Red, Green, Blue, bold, italic)
End Sub
