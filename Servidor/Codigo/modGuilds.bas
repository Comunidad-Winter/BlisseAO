...
Dim DummyString     As String
    ...
    If UserList(FundadorIndex).Stats.UserAtributos(eAtributos.Carisma) < 18 Then
        refError = "Necesitas 18 de carisma para poder fundar un clan."
        Exit Function
    End If
...
Public Function GuildResolve(ByVal UserIndex As Integer) As String
'Khalem
'Khalem
...
Dim GuildMembers() As String
    ...
    End If
        ...
        If GuildI <= 0 Then
         '   Call WriteConsoleMsg(UserIndex, "¡No posees clan!", FontTypeNames.FONTTYPE_GUILD)
            Exit Function
        End If
        
    
        If Not HasFound(UserList(UserIndex).name) Then
         '   Call WriteConsoleMsg(UserIndex, "¡No eres el lider del clan!", FontTypeNames.FONTTYPE_GUILD)
            Exit Function
        End If
  
'Call WriteConsoleMsg(UserIndex, "Cerrando el clan...", FontTypeNames.FONTTYPE_GUILD)
       

'TIENE QUE ECHAR A TODOS LOS DEL CLAN!!!
'/Salirclan

    GuildMembers = guilds(GuildI).GetMemberList()
    Dim i As Byte
            For i = 0 To UBound(GuildMembers)
                    If Not UCase(UserList(UserIndex).name) = GuildMembers(i) Then
                            'obtengo el guildindex
        Call m_EcharMiembroDeClan(-1, GuildMembers(i))
        
                    End If
            Next i

'De acá en más hay que adaptar.

'Call SendData(SendTarget.ToAll, 0, 0, PrepareMessageConsoleMsg("El Clan " & guilds(UserList(userindex).GuildIndex).GuildName & " cerró." & FontTypeNames.FONTTYPE_GUILD))
'Call WriteConsoleMsg(UserIndex, "Cerrando el clan...", FontTypeNames.FONTTYPE_INFOBOLD)
 
Call WriteVar(App.Path & "\guilds\guildsinfo.inf", "GUILD" & UserList(UserIndex).GuildIndex, "Founder", "NADIE")
Call WriteVar(App.Path & "\guilds\guildsinfo.inf", "GUILD" & UserList(UserIndex).GuildIndex, "GuildName", "CLANCERRADO")
...
'Call WriteConsoleMsg(UserIndex, "Desconectando...", FontTypeNames.FONTTYPE_INFOBOLD)

'Call WriteConsoleMsg(UserIndex, "Desconectando...", FontTypeNames.FONTTYPE_GUILD)
