...
'***************************************************
                ...
                If Abs(UserList(UserIndex).Pos.Y - .Pos.Y) <= RANGO_VISION_Y Then
                    ...
                    If Not criminal(UserIndex) Then
                        ...
                        If UserList(UserIndex).flags.Muerto = 0 And UserList(UserIndex).flags.invisible = 0 And _
                            ...
                            If (.AttackUI = 0) Or (.AttackUI <> UserIndex) Then
                                .AttackUI = UserIndex
                                .OrgPos = .Pos
                            End If
    ...
    With Npclist(NpcIndex)
         If (.AttackUI <> 0) Then
             If Distancia(.Pos, .OrgPos) <> 0 Then
                 Call returnToPos(NpcIndex)
             Else
                 .AttackUI = 0
...
Public Sub returnToPos(ByRef npc_Index As Integer)
 
    '
    ' @ maTih.-
   
    Dim mHeading As eHeading
   
    ' // find and fix direction //
    mHeading = FixDirection(Npclist(npc_Index).Pos, FindDirection(Npclist(npc_Index).Pos, Npclist(npc_Index).OrgPos))
   
    ' // move npc char //
    Call MoveNPCChar(npc_Index, mHeading)
End Sub
