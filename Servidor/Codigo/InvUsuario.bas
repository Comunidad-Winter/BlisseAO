            ...
            Do While (Cantidad > 0)
                ...
                If .clase = eClass.Pirat And .Invent.BarcoObjIndex = 476 Then
                    ...
                    If AuxPos.X <> 0 And AuxPos.Y <> 0 Then
                        ...
                        Call UserItemsSound(UserIndex, ObjData(MiObj.ObjIndex).OBJType)
                    End If
                Else
...
With UserList(UserIndex)
    If num > 0 Then
        ...
        If MapData(.Pos.Map, X, Y).ObjInfo.ObjIndex = 0 Or MapData(.Pos.Map, X, Y).ObjInfo.ObjIndex = Obj.ObjIndex Then
            ...
            If MapData(Map, X, Y).ObjInfo.ObjIndex > 0 And (MapData(Map, X, Y).ObjInfo.Amount + Obj.Amount >= 10000) Then
            ...
            Call UserItemsSound(UserIndex, ObjData(Obj.ObjIndex).OBJType)
        Else
            Call WriteConsoleMsg(UserIndex, "No hay espacio en el piso.", FontTypeNames.FONTTYPE_INFO)
        End If
    End If
    ...
    If Obj.ObjIndex > 0 And Obj.ObjIndex <= UBound(ObjData) Then
        ...
        With MapData(Map, X, Y)
            ...
            Else
                ...
                Call modSendData.SendToAreaByPos(Map, X, Y, PrepareMessageObjectCreate(ObjData(Obj.ObjIndex).GrhIndex, X, Y, IIf(NoEsAmbiente(Obj.ObjIndex), ObjData(Obj.ObjIndex).name, "")))
            End If
        End With
    End If
...
On Error GoTo Errhandler
    ...
    With UserList(UserIndex)
        ...
        If Obj.Aura <> 0 Then
            Dim Aura As Byte
                Aura = TieneEstaAura(UserIndex, Obj.Aura)
        
            If Aura <> 0 Then
                Call SetAura(UserIndex, 1, Aura, True)
            End If
        End If
...
On Error GoTo Errhandler
    ...
    With UserList(UserIndex)
        ...
        Select Case Obj.OBJType
            ...
            'Khalem
            If .flags.Metamorfosis = 1 Then Exit Sub 'Metamorfosis
        ...
        'Cargamos las auras de este Objeto
            If ObjData(.Invent.Object(Slot).ObjIndex).Aura <> 0 And .Invent.Object(Slot).Equipped = 1 Then
                Call SetAura(UserIndex, ObjData(.Invent.Object(Slot).ObjIndex).Aura, FindSlotFreeAura(UserIndex), True)
            End If
            ...
            Case eOBJType.otWeapon
                ...
                Else
                    ...
                    Select Case ObjIndex
                        Case CAÑA_PESCA, RED_PESCA, CAÑA_PESCA_NW
                            If .Invent.WeaponEqpObjIndex = CAÑA_PESCA Or .Invent.WeaponEqpObjIndex = RED_PESCA Or .Invent.WeaponEqpObjIndex = CAÑA_PESCA_NW Then
                        ...
                        Case HACHA_LEÑADOR, HACHA_LEÑA_ELFICA, HACHA_LEÑADOR_NW
                            If .Invent.WeaponEqpObjIndex = HACHA_LEÑADOR Or .Invent.WeaponEqpObjIndex = HACHA_LEÑA_ELFICA Or .Invent.WeaponEqpObjIndex = HACHA_LEÑADOR_NW Then
                                Call WriteMultiMessage(UserIndex, eMessages.WorkRequestTarget, eSkill.Talar)
                            Else
                                Call WriteConsoleMsg(UserIndex, "Debes tener equipada la herramienta para trabajar.", FontTypeNames.FONTTYPE_INFO)
                            End If
                            
                        Case PIQUETE_MINERO, PIQUETE_MINERO_NW
                            If .Invent.WeaponEqpObjIndex = PIQUETE_MINERO Or .Invent.WeaponEqpObjIndex = PIQUETE_MINERO_NW Then
                                Call WriteMultiMessage(UserIndex, eMessages.WorkRequestTarget, eSkill.Mineria)
                            Else
                                Call WriteConsoleMsg(UserIndex, "Debes tener equipada la herramienta para trabajar.", FontTypeNames.FONTTYPE_INFO)
                            End If
                            
                        Case MARTILLO_HERRERO, MARTILLO_HERRERO_NW
                            If .Invent.WeaponEqpObjIndex = MARTILLO_HERRERO Or .Invent.WeaponEqpObjIndex = MARTILLO_HERRERO_NW Then
                                Call WriteMultiMessage(UserIndex, eMessages.WorkRequestTarget, eSkill.Herreria)
                            Else
                                Call WriteConsoleMsg(UserIndex, "Debes tener equipada la herramienta para trabajar.", FontTypeNames.FONTTYPE_INFO)
                            End If
                            
                        Case SERRUCHO_CARPINTERO, SERRUCHO_CARPINTERO_NW
                            If .Invent.WeaponEqpObjIndex = SERRUCHO_CARPINTERO Or .Invent.WeaponEqpObjIndex = SERRUCHO_CARPINTERO_NW Then
