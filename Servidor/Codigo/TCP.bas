...
Sub ConnectNewUser(ByVal UserIndex As Integer, ByRef name As String, ByVal UserRaza As eRaza, ByVal UserSexo As eGenero, ByVal UserClase As eClass, _
                     ByVal Hogar As eCiudad, ByVal Head As Integer, ByVal CuentaName As String)
'*************************************************
'Author: Unknown
...
With UserList(UserIndex)
    ...
    If Not PuedeAgregar(CuentaName) Then
        Call WriteErrorMsg(UserIndex, "Sin Slots.")
        Exit Sub
    End If
    ...
    Select Case UserClase
        ...
        Case eClass.Mage 'Standelf
            ' Baculo (Newbie)
            .Invent.Object(Slot).ObjIndex = 862
        Case eClass.Worker
            ' Herramienta (Newbie)
            .Invent.Object(Slot).ObjIndex = RandomNumber(HACHA_LEÑADOR_NW, MARTILLO_HERRERO_NW) 'Standelf
        Case Else
            ' Daga (Newbie)
...
'TonchitoZ: Agrego PJ a la cuenta
Call AgregarPersonaje(CuentaName, name)
...
'Standelf
Dim LastPJ As String, PJS As Integer
LastPJ = UserList(UserIndex).name
PJS = GetVar(App.Path & "\Server.ini", "ESTADISTICAS", "cantidadpjs")

Call WriteVar(App.Path & "\Server.ini", "ESTADISTICAS", "ultimopj", LastPJ)
Call WriteVar(App.Path & "\Server.ini", "ESTADISTICAS", "cantidadpjs", (PJS + 1))
'Standelf
...
On Error GoTo Errhandler
    ...
    'Si está en duelo y logró salir, lo ponemos como si hubiera mandado /SALIRDUELO
    If UserList(UserIndex).flags.EnDuelo = 1 Then _
...
With UserList(UserIndex)
    ...
    If EsGM(UserIndex) Then
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor> El GameMaster " & UserList(UserIndex).name & " ingresó a las tierras de Blisse-AO.", FontTypeNames.FONTTYPE_FIGHT))
    End If
    ...
    Call SendAuras(UserIndex)
    Call Send_Estado(UserIndex)
