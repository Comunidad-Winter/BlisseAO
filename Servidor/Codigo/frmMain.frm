...
Static HH, DH As Integer 'Intervalo de HappyHour y Duración del mismo evento
...
'TonchitoZ Happy Hour for VIP Users
If HH >= IntHappyHour Then
    If DH = DuracionHHour Then
        ExpAndGoldHH (False)
        DH = 0
        HH = 0
    ElseIf DH < DuracionHHour Then
        ExpAndGoldHH (True)
        DH = DH + 1
    End If
End If

If Duelos.Cuenta = True Then Call CuentaRegresiva
...
Static TimerUpdateWeb As Integer
    TimerUpdateWeb = TimerUpdateWeb + 1
    
    If TimerUpdateWeb >= 5 Then
        Update_Statics
        TimerUpdateWeb = 0
    End If
