...
On Error GoTo Errhandler
    ...
    If InMapBounds(Map, X, Y) Then
        With MapData(Map, X, Y)
            If .trigger = eTrigger.AUTORESU Then
                If UserList(UserIndex).flags.Muerto Then Call RevivirUsuario(UserIndex)
...
Function FindDirection(A As WorldPos, B As WorldPos, Optional PuedeAgu As Boolean = False) As eHeading
...
If A.X > B.X And A.Y > B.Y Then
    FindDirection = IIf(r = 1, eHeading.NORTH, eHeading.WEST)
   
ElseIf A.X < B.X And A.Y < B.Y Then
    FindDirection = IIf(r = 1, eHeading.SOUTH, eHeading.EAST)
   
ElseIf A.X < B.X And A.Y > B.Y Then
    FindDirection = IIf(r = 1, eHeading.NORTH, eHeading.EAST)
   
ElseIf A.X > B.X And A.Y < B.Y Then
    FindDirection = IIf(r = 1, eHeading.SOUTH, eHeading.WEST)
   
ElseIf A.X = B.X Then
    FindDirection = IIf(A.Y < B.Y, eHeading.SOUTH, eHeading.NORTH)
   
ElseIf A.Y = B.Y Then
    FindDirection = IIf(A.X < B.X, eHeading.EAST, eHeading.WEST)
 
Else
 
FindDirection = 0 ' this is imposible!
...
If Distancia(A, B) > 1 Then
    ...
    If Not LegalPos(A.Map, A.X, A.Y - 1, PuedeAgu) Then
        ...
        Else
            FindDirection = IIf(r > 1, eHeading.WEST, eHeading.EAST)
        End If
        PV = 1
    ...
    If Not LegalPos(A.Map, A.X - 1, A.Y, PuedeAgu) Then
        ...
        Else
            FindDirection = IIf(r > 1, eHeading.NORTH, eHeading.SOUTH)
        End If
        PV = 1
    End If
...
If PV = 1 Then
    ...
    Select Case FindDirection
        Case EAST
            If Not LegalPos(A.Map, A.X + 1, A.Y) Then FindDirection = eHeading.WEST
       
        Case WEST
            If Not LegalPos(A.Map, A.X - 1, A.Y) Then FindDirection = eHeading.EAST
           
        Case NORTH
            If Not LegalPos(A.Map, A.X, A.Y - 1) Then FindDirection = eHeading.SOUTH
       
        Case SOUTH
            If Not LegalPos(A.Map, A.X, A.Y + 1) Then FindDirection = eHeading.NORTH
...
Function FindDirectionEAO(Pos As WorldPos, Target As WorldPos) As eHeading
'***************************************************
'Author: Unknown
'Last Modification: -
...
'*****************************************************************
    ...
    If Sgn(X) = -1 And Sgn(Y) = 1 Then
        FindDirectionEAO = IIf(RandomNumber(0, 1), eHeading.NORTH, eHeading.EAST)
        Exit Function
    End If
    
    'NW
    If Sgn(X) = 1 And Sgn(Y) = 1 Then
        FindDirectionEAO = IIf(RandomNumber(0, 1), eHeading.WEST, eHeading.NORTH)
        Exit Function
    End If
    
    'SW
    If Sgn(X) = 1 And Sgn(Y) = -1 Then
        FindDirectionEAO = IIf(RandomNumber(0, 1), eHeading.WEST, eHeading.SOUTH)
        Exit Function
    End If
    
    'SE
    If Sgn(X) = -1 And Sgn(Y) = -1 Then
        FindDirectionEAO = IIf(RandomNumber(0, 1), eHeading.SOUTH, eHeading.EAST)
        Exit Function
    End If
