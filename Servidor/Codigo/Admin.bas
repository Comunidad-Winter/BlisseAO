...
'***************************************************
    ...
    Dim tUser, CuentaName As Integer
    ...
    With UserList(bannerUserIndex)
        If tUser <= 0 Then
            ...
            If FileExist(CharPath & UserName & ".chr", vbNormal) Then
                ...
                Else
                    If GetVar(App.Path & "\Cuentas\" & CuentaName & ".bgao", "CUENTA", "BAN") <> "0" Then
                        Call WriteConsoleMsg(bannerUserIndex, "El personaje ya se encuentra baneado.", FontTypeNames.FONTTYPE_INFO)
                    Else
                        Call LogBanFromName(CuentaName, bannerUserIndex, reason)
                        Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("Servidor> " & .name & " ha baneado a " & CuentaName & ".", FontTypeNames.FONTTYPE_SERVER))
                        
                        'ponemos el flag de ban a 1
                        Call WriteVar(App.Path & "\Cuentas\" & CuentaName & ".bgao", "CUENTA", "BAN", "1")
                        'ponemos la pena
                        ...
                        Call LogGM(.name, "BAN a " & CuentaName)
                    End If
                End If
            Else
                Call WriteConsoleMsg(bannerUserIndex, "La cuenta " & CuentaName & " no existe.", FontTypeNames.FONTTYPE_INFO)
            End If
        Else
            ...
            Call LogBan(CuentaName, bannerUserIndex, reason)
