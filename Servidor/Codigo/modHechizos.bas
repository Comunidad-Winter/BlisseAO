...
'***************************************************
    ...
    If HechizoCasteado Then
        With UserList(UserIndex)
            ...
            If Hechizos(SpellIndex).Projectile <> 0 Then Call SendProjectile(UserIndex, .flags.TargetUser, Hechizos(SpellIndex).Projectile, False)
...
End With
        ...
        If Hechizos(HechizoIndex).Projectile <> 0 Then Call SendProjectile(UserIndex, TargetIndex, Hechizos(HechizoIndex).Projectile, False)
End Sub
...
If Hechizos(SpellIndex).Projectile <> 0 And HechizoCasteado = True Then Call SendProjectile(UserIndex, NpcIndex, Hechizos(SpellIndex).Projectile, True)
...
With Npclist(NpcIndex)
    ...
    If Hechizos(SpellIndex).SubeHP = 1 Then
        ...
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("+" & daño, .Char.CharIndex, vbGreen))
    ...
    ElseIf Hechizos(SpellIndex).SubeHP = 2 Then
        ...
        'Hechizo Mata Drakos
        If SpellIndex = SpellMataDragonesIndex Then
            If Npclist(NpcIndex).NPCtype = DRAGON Then
                If Not (UserList(UserIndex).Invent.WeaponEqpObjIndex = BaculoMataDragonesIndex) Then
                    Call WriteConsoleMsg(UserIndex, "Debes tener equipado el Báculo del dragón para poder atacar a esta criatura!", FontTypeNames.FONTTYPE_FIGHT)
                    daño = 1
                End If
            Else
                Call WriteConsoleMsg(UserIndex, "Este Hechizo es solo efectivo contra dragones!", FontTypeNames.FONTTYPE_FIGHT)
                daño = 1
            End If
        Else
            If Npclist(NpcIndex).NPCtype = DRAGON Then
                Call WriteConsoleMsg(UserIndex, "Debes tener equipado el Báculo del dragón y lanzar el hechizo apropiado para poder atacar a esta criatura!", FontTypeNames.FONTTYPE_FIGHT)
                daño = 1
            End If
        End If
        ...
        'Khalem
        If .flags.Metamorfosis = 1 Then daño = daño * 2 'Metamorfosis
                
        Call WriteConsoleMsg(UserIndex, "¡Le has quitado " & daño & " puntos de vida a la criatura!", FontTypeNames.FONTTYPE_FIGHT)
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("-" & daño, .Char.CharIndex, vbRed))
...
End With
    ...
    If Hechizos(SpellIndex).Projectile <> 0 Then Call SendProjectile(UserIndex, NpcIndex, Hechizos(SpellIndex).Projectile, True)
...
With UserList(TargetIndex)
    ...
    If Hechizos(SpellIndex).SubeHP = 1 Then
        ...
        If UserIndex <> TargetIndex Then
            ...
            Call SendData(SendTarget.ToPCArea, TargetIndex, PrepareMessageChatOverHead("+" & daño, UserList(TargetIndex).Char.CharIndex, vbGreen))
        Else
            Call WriteConsoleMsg(UserIndex, "Te has restaurado " & daño & " puntos de vida.", FontTypeNames.FONTTYPE_FIGHT)
            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("+" & daño, UserList(UserIndex).Char.CharIndex, vbGreen))
        End If
    ...
    ElseIf Hechizos(SpellIndex).SubeHP = 2 Then
        ...
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("-" & daño, UserList(UserIndex).Char.CharIndex, vbRed))
...
End With
    ...
    If Hechizos(SpellIndex).Projectile <> 0 Then Call SendProjectile(UserIndex, TargetIndex, Hechizos(SpellIndex).Projectile, False)
