...
Private Enum ClientPacketID
    ...
    GuildFundation
    GuildResolve            '/Cerrarclan Khalem
    ...
    Editame 'Standelf
    MoverItemEnInventario 'Standelf
    GlobalChat 'Standelf, Chat Global
    FaccionChat 'Standelf, Chat entre facciones
    Dame_Promedio 'Standelf, Pedir Promedio de vida
    Save_Character 'Standelf, Guardar Usuario
    Init_Invasion 'Standelf, invasiones
    CuentaCL
    IniciarSubasta 'Standelf, iniciamos una subasta
    OfertarSubasta ' Standelf, Ofertamos en la subasta
    ConsultaSubasta ' Standelf, Si existe una subasta enviamos la Info, sino Abrimos el panel para iniciar una subasta.
    Enter_Duel      '/DUELO
    Exit_Duel       '/SALIRDUELO
    CuentaRegresivaa
    Prestigio
    CentroCanje
    GiveMePower
...
On Error Resume Next
    ...
    Select Case packetID
        ...
        Case ClientPacketID.GuildResolve            '/CerrarCLAN
            Call HandleGuildResolve(UserIndex)
        ...
        Case ClientPacketID.Editame 'Standelf
            Call HandleEditame(UserIndex)
            
        Case ClientPacketID.MoverItemEnInventario  'Standelf
            Call HandleActualizarInventario(UserIndex)
            
        Case ClientPacketID.Save_Character   'Standelf
            Call HandleSaveCharacterPersonal(UserIndex)
            
        Case ClientPacketID.Dame_Promedio 'Standelf
            Call HandleSendPromedio(UserIndex)
            
        Case ClientPacketID.GlobalChat 'Standelf
            Call HandleTalkGlobal(UserIndex)
            
        Case ClientPacketID.FaccionChat 'Standelf
            Call HandleTalkFaccion(UserIndex)
            
        Case ClientPacketID.Init_Invasion  'Standelf Invasiones!!!
            Call HandleInvasion(UserIndex)
       ...
       Case ClientPacketID.IniciarSubasta
        ...
        Case ClientPacketID.ConsultaSubasta
            Call HandleInitSubasta(UserIndex)

        Case ClientPacketID.Enter_Duel
            Call HandleEnterDuel(UserIndex)
            
        Case ClientPacketID.Exit_Duel
            Call HandleExitDuel(UserIndex)
            
        Case ClientPacketID.CuentaRegresivaa
            Call HandleCuentaRegresivaa(UserIndex)
            
        Case ClientPacketID.Prestigio
            Call HandlePrestigio(UserIndex)

        Case ClientPacketID.GiveMePower
            Call HandleGiveMePower(UserIndex)
...
With UserList(UserIndex)
    ...
    Select Case Command
        ...
        Case eGMCommands.DarPermisoMOD          '/AUTORIZAR
            Call HandleAutorizar(UserIndex)
...
On Error GoTo Errhandler
    ...
    UserList(UserIndex).ParticleMeditation = buffer.ReadBoolean()
    ...
    End If
        ...
        If BANCheck(UserName) Then
            Call WriteErrorMsg(UserIndex, "Se te ha prohibido la entrada a Bliss AO a causa de: " & GetVar(App.Path & "\logs\" & "BanDetail.dat", UserName, "Reason") & ", En la fecha: " & GetVar(App.Path & "\logs\" & "BanDetail.dat", UserName, "Fecha"))
...
'***************************************************
    ...
    With UserList(UserIndex).Stats
        .UserAtributos(eAtributos.Fuerza) = MaximoInt(16, 13 + RandomNumber(1, 3) + RandomNumber(1, 2))
        .UserAtributos(eAtributos.Agilidad) = MaximoInt(16, 12 + RandomNumber(1, 3) + RandomNumber(1, 3))
        .UserAtributos(eAtributos.Inteligencia) = MaximoInt(17, 13 + RandomNumber(1, 3) + RandomNumber(1, 2))
        .UserAtributos(eAtributos.Carisma) = MaximoInt(16, 12 + RandomNumber(1, 3) + RandomNumber(1, 3))
        .UserAtributos(eAtributos.Constitucion) = 16 + RandomNumber(1, 1) + RandomNumber(1, 1)
    End With
...
On Error GoTo Errhandler
    ...
    UserList(UserIndex).ParticleMeditation = buffer.ReadBoolean()
        ...
        If Not VersionOK(version) Then
            Call WriteErrorMsg(UserIndex, "Esta versión del juego es obsoleta, la versión correcta es la " & ULTIMAVERSION & ". La misma se encuentra disponible en www.argentumonline.com.ar")
        Else
            Call ConnectNewUser(UserIndex, UserName, race, gender, Class, homeland, Head, CuentaName)
        End If
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        'Drag&Drop
        Dim Valido As Boolean, TempDrop As WorldPos
        
        TempDrop.map = UserList(UserIndex).Pos.map
        TempDrop.X = X
        TempDrop.Y = Y
        Valido = LegalPos(TempDrop.map, TempDrop.X + 1, TempDrop.Y) = False And _
                    LegalPos(TempDrop.map, TempDrop.X - 1, TempDrop.Y) = False And _
                    LegalPos(TempDrop.map, TempDrop.X, TempDrop.Y - 1) = False And _
                    LegalPos(TempDrop.map, TempDrop.X, TempDrop.Y + 1) = False
        
        If Valido = True Then
            Call WriteConsoleMsg(UserIndex, "La posición en la que intenta tirar el ítem es inválida.", FontTypeNames.FONTTYPE_WARNING)
            Exit Sub
        End If
        
        
        If MapData(UserList(UserIndex).Pos.map, X, Y).Blocked Then
            Call WriteConsoleMsg(UserIndex, "La posición en la que intenta tirar el ítem es inválida.", FontTypeNames.FONTTYPE_WARNING)
            Exit Sub
        End If
        
        If Distancia(TempDrop, UserList(UserIndex).Pos) > 4 Then
            Call WriteConsoleMsg(UserIndex, "La posición en la que intenta tirar el ítem está demasiado lejos.", FontTypeNames.FONTTYPE_WARNING)
            Exit Sub
        End If
        ...
        Else
            ...
            If Slot <= MAX_INVENTORY_SLOTS And Slot > 0 Then
                ...
                Call DropObj(UserIndex, Slot, Amount, .Pos.map, X, Y)
            End If
        End If
    End With
End Sub
        ...
        Select Case Skill
            Case eSkill.Proyectiles
                ...
                If tU > 0 Then
                    ...
                    'Attack!
                    Atacked = UsuarioAtacaUsuario(UserIndex, tU, True)
                ...
                ElseIf tN > 0 Then
                    ...
                    If Npclist(tN).Attackable <> 0 Then
                        ...
                        'Attack!
                        Atacked = UsuarioAtacaNpc(UserIndex, tN, True)
                    End If
                End If
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        Call WriteConsoleMsg(UserIndex, "Número de usuarios: " & CStr(Count) & ". Record de usuarios Online: " & val(recordusuarios), FontTypeNames.FONTTYPE_INFO)
    End With
End Sub
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If .flags.EnDuelo = 1 Then
            Call WriteConsoleMsg(UserIndex, "No puedes salir estando en duelo.", FontTypeNames.FONTTYPE_WARNING)
            Exit Sub
        End If
...
Private Sub HandleEditame(ByVal UserIndex As String)
'***************************************************
'Author: Standelf
'Last Modification: 16/05/2010
'***************************************************
    With UserList(UserIndex)
        'Remove packet ID
        Call .incomingData.ReadByte
    ...
    '    If Not EsGM(UserIndex) Then Exit Sub
        ...
        Dim i As Integer
            If UserList(UserIndex).Stats.ELV < STAT_MAXELV Then
                For i = UserList(UserIndex).Stats.ELV To STAT_MAXELV
        ...
        For i = 1 To NUMSKILLS
            UserList(UserIndex).Stats.UserSkills(i) = 99
            DoEvents
        Next i
        ...
        ' Informamos
        Call WriteConsoleMsg(UserIndex, "Has sido editado correctamente.", FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    End With
...
Private Sub HandleGuildResolve(ByVal UserIndex As Integer)

Call modGuilds.GuildResolve(UserIndex)
...
'25/05/2010: TonchitoZ, Adapto las advertencias (PENAS) a modo cuenta
'*******************************************************
    If UserList(UserIndex).incomingData.length < 5 Then
        Err.Raise UserList(UserIndex).incomingData.NotEnoughDataErrCode
        Exit Sub
...
On Error GoTo Errhandler
    With UserList(UserIndex)
        ...
        Dim UserName, CuentaName As String
        Dim reason As String
        ...
        If (Not .flags.Privilegios And PlayerType.RoleMaster) <> 0 And (Not .flags.Privilegios And PlayerType.User) <> 0 Then
            ...
            Else
                ...
                Else
                    ...
                    If FileExist(CharPath & UserName & ".chr", vbNormal) Then
                        CuentaName = GetVar(CharPath & UserName & ".chr", "INIT", "CUENTA")
                        Count = val(GetVar(App.Path & "\Cuentas\" & CuentaName & ".bgao", "CUENTA", "ADVERTENCIAS"))
                        Call WriteVar(App.Path & "\Cuentas\" & CuentaName & ".bgao", "CUENTA", "ADVERTENCIAS", Count + 1)
                        Call WriteVar(App.Path & "\Cuentas\" & CuentaName & ".bgao", "ADVERTENCIAS", "ADV" & Count + 1, LCase$(.name) & ": ADVERTENCIA por: " & LCase$(reason) & " " & Now)
                        
                        Call WriteConsoleMsg(UserIndex, "Has advertido a " & UCase$(UserName) & ".", FontTypeNames.FONTTYPE_INFO)
                        Call LogGM(.name, " advirtió a " & UserName & ", perteneciente a la cuenta: " & CuentaName)
                    End If
                End If
            End If
        End If
...
Public Sub HandleAutorizar(ByVal UserIndex As Integer)
    If UCase$(UserList(UserIndex).name) <> "Dunkan" Then Exit Sub
    
    With UserList(UserIndex)
        'Remove packet ID
        Call .incomingData.ReadByte
        Dim GameMaster As String
                GameMaster = .incomingData.ReadASCIIString
                UserList(NameIndex(GameMaster)).flags.Autorizado = True
                WriteConsoleMsg NameIndex(GameMaster), "Fuiste autorizado a usar el comando /MOD por única vez. Recuerda que los usuarios serán informados sobre el uso de esta herramienta", FONTTYPE_FIGHT
    End With
End Sub
...
On Error GoTo Errhandler
    With UserList(UserIndex)
       ...
       ' If UCase$(.name) = "STANDELF" Then .flags.Autorizado = True
            ...
            Else
                ...
                Select Case opcion
                    Case eEditOptions.eo_Gold
                        If UserList(UserIndex).flags.Autorizado = False Then
                            Call WriteConsoleMsg(UserIndex, "Debes ser autorizado previamente por un Administrador para realizar esta acción.", FONTTYPE_FIGHT)
                        Else
                            If val(Arg1) <= MAX_ORO_EDIT Then
                                ...
                                UserList(UserIndex).flags.Autorizado = False
                            Else
        ...
        If .flags.Privilegios And (PlayerType.Admin Or PlayerType.SemiDios Or PlayerType.Dios) Then
            ...
            If tIndex > 0 Then
                If Slot > 0 And Slot <= UserList(tIndex).CurrentInventorySlots Then
                    ...
                    End If
                Else
                    Call WriteConsoleMsg(UserIndex, "Slot Inválido. Adquiera una Mochila para expandir su inventario.", FontTypeNames.FONTTYPE_TALK)
                End If
            Else
...
Public Sub WriteObjectCreate(ByVal UserIndex As Integer, ByVal GrhIndex As Integer, ByVal X As Byte, ByVal Y As Byte, ByVal name As String)
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Writes the "ObjectCreate" message to the given user's outgoing data buffer
'***************************************************
On Error GoTo Errhandler
    Call UserList(UserIndex).outgoingData.WriteASCIIStringFixed(PrepareMessageObjectCreate(GrhIndex, X, Y, name))
Exit Sub
...
On Error GoTo Errhandler
    ...
    With UserList(UserIndex).outgoingData
        ...
        For i = LBound(guildList()) To UBound(guildList())
            'Khalem
            If Not guildList(i) = "CLANCERRADO" Then
...
On Error GoTo Errhandler
    With UserList(UserIndex).outgoingData
        ...
        Call .WriteByte(Slot)
        Call .WriteByte(getMaxInventorySlots(UserIndex))
        ...
        If ObjIndex <> 0 Then
                If ClasePuedeUsarItem(UserIndex, ObjIndex) And _
                   ...
                   FaccionPuedeUsarItem(UserIndex, ObjIndex) Then PuedeUsar = True
                   
            If ItemNewbie(ObjIndex) = True Then
                If EsNewbie(UserIndex) Then PuedeUsar = True
            End If
        Else
            PuedeUsar = False
        End If
        Call .WriteBoolean(PuedeUsar)
...
Public Function PrepareMessageObjectCreate(ByVal GrhIndex As Integer, ByVal X As Byte, ByVal Y As Byte, Optional ByVal name As String = "") As String
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'prepares the "ObjectCreate" message and returns it
...
Public Sub HandleSaveCharacterPersonal(ByVal UserIndex As Integer)
    With UserList(UserIndex)
        'Remove Packet ID
        Call .incomingData.ReadByte

        Call SaveUser(UserIndex, CharPath & UCase$(UserList(UserIndex).name) & ".chr", True)

        Call WriteConsoleMsg(UserIndex, "Personaje guardado correctamente.", FontTypeNames.FONTTYPE_SERVER)
    End With
End Sub

Public Sub HandleSendPromedio(ByVal UserIndex As Integer)
    With UserList(UserIndex)
        'Remove Packet ID
        Call .incomingData.ReadByte
        Call WriteConsoleMsg(UserIndex, "Server> Tu promedio es de: " & UserList(UserIndex).Stats.MaxHp / UserList(UserIndex).Stats.ELV & ".", FONTTYPE_SERVER)
    End With
End Sub

Public Sub HandleActualizarInventario(ByVal UserIndex As Integer)
Dim Arg1 As Byte, Arg2 As Byte

                
    With UserList(UserIndex)
    'removepacket
    Call .incomingData.ReadByte
        Arg1 = .incomingData.ReadByte
        Arg2 = .incomingData.ReadByte
        
    If Arg1 = Arg2 Then Exit Sub
    
    If val(Arg1) <= 0 Or val(Arg1) > getMaxInventorySlots(UserIndex) Then
        Call WriteConsoleMsg(UserIndex, "Slot Inválido.", FONTTYPE_SERVER)
        Exit Sub
    End If
    
    If val(Arg2) <= 0 Or val(Arg2) > getMaxInventorySlots(UserIndex) Then
        Call WriteConsoleMsg(UserIndex, "Slot Inválido.", FONTTYPE_SERVER)
        Exit Sub
    End If
    
                    If .Invent.Object(Arg2).ObjIndex <> 0 Then
                        Dim tpm1 As Integer, tpm2 As Integer, tpm3 As Integer
                        tpm1 = .Invent.Object(Arg2).ObjIndex
                        tpm2 = .Invent.Object(Arg2).Amount
                        tpm3 = .Invent.Object(Arg2).Equipped
                        
                        .Invent.Object(Arg2).ObjIndex = .Invent.Object(Arg1).ObjIndex
                        .Invent.Object(Arg2).Amount = .Invent.Object(Arg1).Amount
                        .Invent.Object(Arg2).Equipped = .Invent.Object(Arg1).Equipped
                        
                        .Invent.Object(Arg1).ObjIndex = tpm1
                        .Invent.Object(Arg1).Amount = tpm2
                        .Invent.Object(Arg1).Equipped = tpm3
                        
                        UpdateUserInv False, UserIndex, Arg1
                        UpdateUserInv False, UserIndex, Arg2
                    Else
                        .Invent.Object(Arg2).ObjIndex = .Invent.Object(Arg1).ObjIndex
                        .Invent.Object(Arg2).Amount = .Invent.Object(Arg1).Amount
                        .Invent.Object(Arg2).Equipped = .Invent.Object(Arg1).Equipped
                        .Invent.Object(Arg1).ObjIndex = 0
                        .Invent.Object(Arg1).Amount = 0
                        .Invent.Object(Arg1).Equipped = 0
...
Public Sub HandleTalkGlobal(ByVal UserIndex As Integer)
    Dim tmpMessage
    ...
    With UserList(UserIndex)
        ...
        tmpMessage = .incomingData.ReadASCIIString
    End With
    
    If Not CaracterInvalido(tmpMessage, Asc("~")) Then
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("[Global] " & UserList(UserIndex).name & "> " & tmpMessage, FontTypeNames.FONTTYPE_INFO))
    Else
        Call WriteConsoleMsg(UserIndex, "Server> Hay caracteres inválidos en su texto.", FONTTYPE_SERVER)
    End If

End Sub

Public Sub HandleInvasion(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If NpcIndex = -1 Then
            Call Detener_Invasion(UserIndex)
            Exit Sub
        End If
        
        If Check_Hay_Invasion = True Then
            Call WriteConsoleMsg(UserIndex, "Server> Ya hay una invasión actualmente, deberás esperar a que termine esta invasión.", FONTTYPE_SERVER)
            Exit Sub
        End If
        ...
        Dim i As Byte
            For i = 1 To 20
                Invasion(i) = SpawnNpc(NpcIndex, .Pos, True, False)
            Next i
            
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Una invasión de " & Npclist(Invasion(1)).name & " Comenzó en " & MapInfo(.Pos.map).name & " (Mapa: " & .Pos.map & ")", FontTypeNames.FONTTYPE_DIOS))
       
        If NpcIndex <> 0 Then
            Call LogGM(.name, "¡¡ Inició una Invasion !! Sumoneó a 20 " & Npclist(NpcIndex).name & " en el mapa " & .Pos.map)
        End If
        
    End With
End Sub

Public Sub HandleTalkFaccion(ByVal UserIndex As Integer)
    Dim tmpMessage
    ...
    If Not CaracterInvalido(tmpMessage, Asc("~")) Then
        ...
        If UserList(UserIndex).Faccion.ArmadaReal = 1 Then
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("[Armada Real] " & UserList(UserIndex).name & "> " & tmpMessage, FontTypeNames.FONTTYPE_CONSEJO))
        
        'Fuerza Caos
        ElseIf UserList(UserIndex).Faccion.FuerzasCaos = 1 Then
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("[Fuerza Caos] " & UserList(UserIndex).name & "> " & tmpMessage, FontTypeNames.FONTTYPE_CONSEJOCAOS))
        
        'Rata Inmunda que manda por mandar
        Else
            Call WriteConsoleMsg(UserIndex, "Server> Para usar el Chat faccionario deberás pertenecer a la Armada Real o a las Fuerzas del Caos.", FONTTYPE_SERVER)
        End If
...
Public Sub WriteParaTime(ByVal UserIndex As Integer, ByVal Paralisis As Byte)
'***************************************************
'Author: Standelf
'Last Modification: 22/05/2010
'***************************************************
On Error GoTo Errhandler
    Call UserList(UserIndex).outgoingData.WriteASCIIStringFixed(PrepareMessageParaTime(Paralisis))
Exit Sub
...
Public Function PrepareMessageParaTime(ByVal Paralisis As Byte) As String
'***************************************************
'Author: Standelf
'Last Modification: 22/05/2010
'***************************************************
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.ParaTime)
        Call .WriteByte(Paralisis)
        
        PrepareMessageParaTime = .ReadASCIIStringFixed(.length)
    End With
End Function
...
With UserList(UserIndex)
    ...
    Select Case Command
        ...
        Case "BANKSTART"
            ...
            Call IniciarBoveda(UserIndex, CuentaName)
            Exit Sub
        ...
        Case "BANKDEPOSIT"
            ...
            Call DepositarBoveda(UserIndex, CuentaName, Slot, Cantidad)
            Exit Sub
        ...
        Case "BANKRETIRAR"
            ...
            Cantidad2 = .incomingData.ReadInteger
            
            Call RetirarBoveda(UserIndex, CuentaName, Slot2, Cantidad2)
            Exit Sub
    End Select
End With
End Sub
Public Sub EnviarCuenta(ByVal UserIndex As Integer, ByVal PJ1 As String, ByVal PJ2 As String, ByVal PJ3 As String, ByVal PJ4 As String, _
                           ByVal PJ5 As String, ByVal PJ6 As String, ByVal PJ7 As String, ByVal PJ8 As String, ByVal Premium As Boolean, ByVal CantPjs As Byte, ByVal estado As Byte)
With UserList(UserIndex).outgoingData
...
End With
End Sub
Public Sub EnviarBoveda(ByVal UserIndex As Integer)
With UserList(UserIndex).outgoingData
    .WriteByte (ServerPacketID.CuentaSV)
    .WriteASCIIString ("BANKITEMS")
    .WriteASCIIString (Cuenta.ItemBov(1).name)
    .WriteLong (Cuenta.ItemBov(1).Cantidad)
    .WriteInteger (Cuenta.ItemBov(1).obInd.ObjIndex)
    .WriteASCIIString (Cuenta.ItemBov(2).name)
    .WriteLong (Cuenta.ItemBov(2).Cantidad)
    .WriteInteger (Cuenta.ItemBov(2).obInd.ObjIndex)
    .WriteASCIIString (Cuenta.ItemBov(3).name)
    .WriteLong (Cuenta.ItemBov(3).Cantidad)
    .WriteInteger (Cuenta.ItemBov(3).obInd.ObjIndex)
    .WriteASCIIString (Cuenta.ItemBov(4).name)
    .WriteLong (Cuenta.ItemBov(4).Cantidad)
    .WriteInteger (Cuenta.ItemBov(4).obInd.ObjIndex)
    .WriteASCIIString (Cuenta.ItemBov(5).name)
    .WriteLong (Cuenta.ItemBov(5).Cantidad)
    .WriteInteger (Cuenta.ItemBov(5).obInd.ObjIndex)
    .WriteASCIIString (Cuenta.ItemBov(6).name)
    .WriteLong (Cuenta.ItemBov(6).Cantidad)
    .WriteInteger (Cuenta.ItemBov(6).obInd.ObjIndex)
    .WriteASCIIString (Cuenta.ItemBov(7).name)
    .WriteLong (Cuenta.ItemBov(7).Cantidad)
    .WriteInteger (Cuenta.ItemBov(7).obInd.ObjIndex)
    .WriteASCIIString (Cuenta.ItemBov(8).name)
    .WriteLong (Cuenta.ItemBov(8).Cantidad)
    .WriteInteger (Cuenta.ItemBov(8).obInd.ObjIndex)
    .WriteASCIIString (Cuenta.ItemBov(9).name)
    .WriteLong (Cuenta.ItemBov(9).Cantidad)
    .WriteInteger (Cuenta.ItemBov(9).obInd.ObjIndex)
    .WriteASCIIString (Cuenta.ItemBov(10).name)
    .WriteLong (Cuenta.ItemBov(10).Cantidad)
    .WriteInteger (Cuenta.ItemBov(10).obInd.ObjIndex)
End With
End Sub

Public Sub WriteSendClima(ByVal UserIndex As Integer, estado As Byte)
    With UserList(UserIndex).outgoingData
        Call .WriteByte(ServerPacketID.SendClima)
        Call .WriteByte(estado)
    End With
End Sub

Public Sub WriteCentroCanje(ByVal UserIndex As Integer)
    With UserList(UserIndex).outgoingData
        Call .WriteByte(ServerPacketID.CentroCanje)
    End With
End Sub

Public Function PrepareUpdateClima(ByVal estado As Byte)
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.UpdateClima)
        Call .WriteByte(estado)
        PrepareUpdateClima = .ReadASCIIStringFixed(.length)
    End With
End Function

Public Function PrepareCreateProjectile(ByVal AtacanteIndex As Integer, VictimaIndex As Integer, Projectile As Byte, npc As Boolean, fallo As Boolean)
'***************************************************
'Author: Standelf
...
'***************************************************
    ...
    With auxiliarBuffer
        ...
        Call .WriteInteger(UserList(AtacanteIndex).Char.CharIndex)
        
        If VictimaIndex = 0 Then
            Call .WriteInteger(UserList(AtacanteIndex).Char.CharIndex)
        Else
            If npc = True Then
                Call .WriteInteger(Npclist(VictimaIndex).Char.CharIndex)
            Else
                Call .WriteInteger(UserList(VictimaIndex).Char.CharIndex)
            End If
        End If
        
        Call .WriteByte(Projectile)
        Call .WriteBoolean(fallo)
        
        PrepareCreateProjectile = .ReadASCIIStringFixed(.length)
    End With
    
End Function

Public Function PrepareAuraSet(ByVal AuraSetIndex As Integer, ByVal Slot As Byte) As String
'***************************************************
'Author: Standelf
'Last Modification: 27/05/2010
'***************************************************
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.AuraSet)
        Call .WriteInteger(UserList(AuraSetIndex).Char.CharIndex)
        Call .WriteByte(UserList(AuraSetIndex).Char.Aura(Slot))
        Call .WriteByte(Slot)
        
        PrepareAuraSet = .ReadASCIIStringFixed(.length)
    End With
End Function
...
Dim MembersOnline(1 To PARTY_MAXMEMBERS) As Integer, i As Byte
    With UserList(UserIndex)
        Call Parties(.PartyIndex).ObtenerMiembrosOnline(MembersOnline())
        
        Call .outgoingData.WriteByte(ServerPacketID.PartyDetail) 'Enviamos el ID
        Call .outgoingData.WriteByte(PARTY_MAXMEMBERS)

        For i = 1 To PARTY_MAXMEMBERS
            If MembersOnline(i) <> 0 Then
                Call .outgoingData.WriteASCIIString(UserList(MembersOnline(i)).name)
                Call .outgoingData.WriteLong(UserList(MembersOnline(i)).Stats.Exp)
                Call .outgoingData.WriteByte(UserList(MembersOnline(i)).Stats.ELV)
                Call .outgoingData.WriteInteger(UserList(MembersOnline(i)).Char.Head)

            Else
                Call .outgoingData.WriteASCIIString("Vacio")
            End If
        Next i
    End With
End Sub

Public Sub WritePartyKicked(ByVal UserIndex As Integer)
    With UserList(UserIndex).outgoingData
        Call .WriteByte(ServerPacketID.PartyKicked)
    End With
End Sub



Public Sub WritePartyExit(ByVal UserIndex As Integer, MemberList As Byte)
    With UserList(UserIndex).outgoingData
        Call .WriteByte(ServerPacketID.PartyExit)
        Call .WriteByte(MemberList)
    End With
End Sub
...
Public Sub HandleInitSubasta(ByVal UserIndex As Integer)
'***************************************************
'Author: Standelf
...
Public Sub SendProjectile(ByVal AtacanteIndex As Integer, VictimaIndex As Integer, ByVal Projectile As Byte, ByVal npc As Boolean, Optional ByVal fallo As Boolean = False)
    If Not (IsNumeric(AtacanteIndex) And IsNumeric(VictimaIndex) And IsNumeric(Projectile)) Then Exit Sub
    
    If npc = True Then
        Call modSendData.SendToMap(UserList(AtacanteIndex).Pos.map, PrepareCreateProjectile(AtacanteIndex, VictimaIndex, Projectile, npc, fallo))
    Else
        Call modSendData.SendToMap(UserList(AtacanteIndex).Pos.map, PrepareCreateProjectile(AtacanteIndex, VictimaIndex, Projectile, npc, fallo))
    End If
...
Public Sub WriteCreateOrUpdate(ByVal UserIndex As Integer)
    With UserList(UserIndex).outgoingData
        Call .WriteByte(ServerPacketID.CreateOrUpdateStats)
        Call .WriteASCIIString("tnefR0")
        ...
        If UserList(UserIndex).GuildIndex <> 0 Then
            Call .WriteASCIIString(modGuilds.GuildName(UserList(UserIndex).GuildIndex))
        Else
            Call .WriteASCIIString("-")
        End If
        ...
        If UserList(UserIndex).Faccion.ArmadaReal = 1 Then
            Call .WriteASCIIString("Real")
        ElseIf UserList(UserIndex).Faccion.FuerzasCaos = 1 Then
            Call .WriteASCIIString("Caos")
        Else
            Call .WriteASCIIString("-")
        End If
        
        Call .WriteASCIIString(ListaClases(UserList(UserIndex).clase))
        Call .WriteASCIIString(ListaRazas(UserList(UserIndex).raza))
        
        Call .WriteInteger(UserList(UserIndex).Faccion.CiudadanosMatados + UserList(UserIndex).Faccion.CriminalesMatados)
        
        
        Call .WriteInteger(UserList(UserIndex).PrestigioR)
        
        If UserList(UserIndex).desc = "" Then
            Call .WriteASCIIString("-")
        Else
            Call .WriteASCIIString(UserList(UserIndex).desc)
        End If
...
Public Function PrepareMessageDuelStart(ByVal ID As Integer) As String
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.DuelStart)
        Call .WriteByte(val(ID)) 'Empieza la cuenta, inmovilizamos
        
        PrepareMessageDuelStart = .ReadASCIIStringFixed(.length)
    End With
End Function

Public Function PrepareMessageCountStart(ByVal Count As Integer) As String
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.CountStart)
        Call .WriteByte(val(Count))
        
        PrepareMessageCountStart = .ReadASCIIStringFixed(.length)
    End With
End Function

Public Function PrepareMessageSendParticle(ByVal CharIndex As Integer, ByVal Particle As Byte, ByVal loops As Byte, Optional ByVal Meditacion As Byte = 0) As String
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.Particles)
        Call .WriteInteger(CharIndex)
        Call .WriteByte(Particle)
        Call .WriteByte(loops)
        Call .WriteByte(Meditacion)
        
        PrepareMessageSendParticle = .ReadASCIIStringFixed(.length)
    End With
End Function


Public Sub HandleEnterDuel(ByVal UserIndex As Integer)
    With UserList(UserIndex)
        Call .incomingData.ReadByte
    End With
    
    If MapInfo(Duelos.map).NumUsers >= 2 Then 'Mapa Con más de 2 users
        Call WriteConsoleMsg(UserIndex, "Server> Ya hay 2 usuarios en duelo, Espera que uno termine para poder entrar.", FONTTYPE_SERVER)
        Exit Sub
    ElseIf UserList(UserIndex).Stats.ELV < 30 Then
        Call WriteConsoleMsg(UserIndex, "Server> Para participar de duelos debes ser nivel 30.", FONTTYPE_SERVER)
        Exit Sub
    ElseIf UserList(UserIndex).flags.Muerto = 1 Then 'Si está muerto
        Call WriteConsoleMsg(UserIndex, "Server> No puedes participar de un duelo estando muerto.", FONTTYPE_SERVER)
        Exit Sub
    ElseIf UserList(UserIndex).Pos.map = Duelos.map Then
        Call WriteConsoleMsg(UserIndex, "Server> Ya estás en la arena de duelo.", FONTTYPE_SERVER)
        Exit Sub
    ElseIf Duelos.Usuario1 = UserIndex Or Duelos.Usuario2 = UserIndex Then
        Call WriteConsoleMsg(UserIndex, "Server> No puedes iniciar un duelo contra ti mismo.", FONTTYPE_SERVER)
        Exit Sub
    Else 'Puede Duelear
        With Duelos
            If .Usuario1 = 0 Then 'Si el Slot está vacio ocupa el primer Slot
                .Usuario1 = UserIndex
                .Pos1 = UserList(UserIndex).Pos
            Else 'Si el Slot está Ocupado ocupa el segundo Slot
                .Usuario2 = UserIndex
                .Pos2 = UserList(UserIndex).Pos
            End If
            
            UserList(UserIndex).flags.EnDuelo = 1
                            
            Call WarpUserChar(UserIndex, Duelos.map, RandomNumber(35, 62), RandomNumber(42, 58), True)
            
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(UserList(UserIndex).name & " ingresó al salón de duelos y espera a su contrincante.", FontTypeNames.FONTTYPE_FIGHT))

            If .Usuario1 <> 0 And .Usuario2 <> 0 Then _
                Call Iniciar_Duelo
        End With
    End If
End Sub

Public Sub HandleCuentaRegresivaa(ByVal UserIndex As Integer)
    With UserList(UserIndex)
        Call .incomingData.ReadByte
        Call SendData(SendTarget.ToAll, 0, PrepareMessageCountStart(.incomingData.ReadByte()))
    End With
End Sub

Public Sub HandleGiveMePower(ByVal UserIndex As Integer)
    With UserList(UserIndex)
        'Remove
        Call .incomingData.ReadByte
        ...
        Dim X As Integer, Y As Integer
            ...
            If UserList(UserIndex).Stats.UserSkills(eSkill.Supervivencia) >= 60 Then
                If MapData(.Pos.map, X, Y).NpcIndex <> 0 Then
                    .outgoingData.WriteByte ServerPacketID.ShowStatsBars
                    .outgoingData.WriteInteger Npclist(MapData(.Pos.map, X, Y).NpcIndex).Char.CharIndex
                    .outgoingData.WriteLong Npclist(MapData(.Pos.map, X, Y).NpcIndex).Stats.MinHp
                    .outgoingData.WriteLong Npclist(MapData(.Pos.map, X, Y).NpcIndex).Stats.MaxHp
                    .outgoingData.WriteASCIIString Npclist(MapData(.Pos.map, X, Y).NpcIndex).name
            ...
            If MapData(.Pos.map, X, Y).UserIndex <> 0 Then
                If .GuildIndex = UserList(MapData(.Pos.map, X, Y).UserIndex).GuildIndex Then
                    .outgoingData.WriteByte ServerPacketID.ShowStatsBars
                    .outgoingData.WriteInteger UserList(MapData(.Pos.map, X, Y).UserIndex).Char.CharIndex
                    .outgoingData.WriteLong UserList(MapData(.Pos.map, X, Y).UserIndex).Stats.MinHp
                    .outgoingData.WriteLong UserList(MapData(.Pos.map, X, Y).UserIndex).Stats.MaxHp
                    .outgoingData.WriteASCIIString UserList(MapData(.Pos.map, X, Y).UserIndex).name
...
Public Sub HandlePrestigio(ByVal UserIndex As Integer)
Dim Tmp As Byte, tmpIndex As Integer, Cantidad As Integer
    With UserList(UserIndex)
        Call .incomingData.ReadByte
        ...
        Select Case Tmp
            Case 0 'Pedir prestigio
                Call DamePrestigio(UserIndex)
            Case 1, 2
                tmpIndex = NameIndex(.incomingData.ReadASCIIString())
            
            If tmpIndex <= 0 Then
                Call WriteConsoleMsg(UserIndex, "Usuario offline.", FontTypeNames.FONTTYPE_INFO)
            Else
                Cantidad = .incomingData.ReadInteger()
                If Tmp = 1 Then Call DarPrestigioRepu(UserIndex, tmpIndex, Cantidad)
                If Tmp = 2 Then Call DarPrestigioCanje(UserIndex, tmpIndex, Cantidad)
            End If
        End Select
    End With
End Sub

Public Sub HandleExitDuel(ByVal UserIndex As Integer)
    With UserList(UserIndex)
        Call .incomingData.ReadByte
    End With
    If UserList(UserIndex).flags.EnDuelo = 1 Then
        Call Rendir_Duelo(UserIndex)
    ElseIf UserList(UserIndex).flags.EnDuelo = 0 And UserList(UserIndex).Pos.map = Duelos.map Then 'No está en duelo, pero está en el mapa.
        Call WriteConsoleMsg(UserIndex, "Server> No estás en duelo, pero no puedes permanecer en este mapa, serás llevado a tu ciudad natal.", FONTTYPE_SERVER)

                    Call WarpUserChar(UserIndex, 1, 50, 50, True)

    Else 'No está en duelo
        Call WriteConsoleMsg(UserIndex, "Server> No estás en duelo.", FONTTYPE_SERVER)
    End If
End Sub
